...
function Index() {
    ...
    const [raiz, setRaiz] = useState(null); // Nuevo estado para almacenar la raíz
    ...
    const handleSubmit = (event) => {
        ...
        .then(data => {
            setDatos(data.iteraciones); // Actualiza los datos de las iteraciones
            setRaiz(data.raiz); // Actualiza la raíz
            console.log(data); 
        })
    ...
    return (
        <div>
            <h1 style={{ textAlign: 'center' }}>BISECCIÓN</h1>
            ...
            {datos && <CustomizedTables rows={datos} raiz={raiz} />} {/* Pasar la raíz como propiedad al componente CustomizedTables */}
        </div>
    );
}
